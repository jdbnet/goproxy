...
func TestSkip(t *testing.T) {
	...
	t.Setenv("GOPROXY_NO_UPDATE", "1")
	if Skip(Config{Enabled: true}, "1.0.0") != "GOPROXY_NO_UPDATE" {
...
func TestCheckReplacesOnChecksumChange(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc("/goproxy-amd64", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write(newBin)
	})
	mux.HandleFunc("/goproxy-amd64.sha256", func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(hex.EncodeToString(newSum[:]) + "  goproxy-amd64\n"))
	})
	srv := httptest.NewServer(mux)
	t.Cleanup(srv.Close)
	...
	ok, err := Check(context.Background(), Config{Enabled: true, URL: srv.URL + "/goproxy-amd64", AllowDev: true}, "1.0.0", t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	if !ok {
...
func TestDownloadRejectsNonELF(t *testing.T) {
	...
	dest := filepath.Join(t.TempDir(), "goproxy.new")
