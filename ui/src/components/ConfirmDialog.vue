  ...
  <Teleport to="body">
    ...
    >
      ...
      <div
        role="alertdialog"
        aria-modal="true"
        :aria-labelledby="confirmState.open ? 'confirm-title' : undefined"
        :aria-describedby="confirmState.message ? 'confirm-message' : undefined"
        class="card relative z-10 w-full max-w-md shadow-xl"
      >
        <h2 id="confirm-title" class="text-lg font-semibold text-heading">{{ confirmState.title }}</h2>
        <p v-if="confirmState.message" id="confirm-message" class="mt-2 text-sm text-muted">{{ confirmState.message }}</p>
